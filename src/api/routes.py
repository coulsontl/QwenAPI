...
async def _handle_chat_with_retry(data: Dict[str, Any], request: Request, raw_body: bytes, model: str, stream: bool, attempt: int):
    ...
    try:
        auth_token = current_token.access_token
        passthrough_headers = {
        ...
        if _version_manager:
            user_agent = await _version_manager.get_user_agent_async()
            passthrough_headers['User-Agent'] = user_agent
            passthrough_headers['X-DashScope-UserAgent'] = user_agent
